...
public:
  ...
  /*!
   * \brief Prevents falling through to another window when looking up keys
   */
  virtual bool GetFallthrough() const = 0;

  /*!
   * \brief Returns true when keymap input is allowed, such as when the app is
   *        in the foreground
   */
  virtual bool AcceptsKeymapInput() const = 0;
};
